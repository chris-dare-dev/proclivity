...
First read **in full** the product-neutral canon: `.Codex/references/frontend-design-language.md`.
...
Then read the **repo-local house-thesis overlay** under `.Codex/references/frontend-uplift/`
...
For motion specifics also consult `.Codex/references/frontend-uplift-motion-vocabulary.md`
(`MOT-*`, surface model §0) and `.Codex/references/frontend-uplift-experiential-motion.md`
(`EXP-*`, recipes) as needed. For library / font / exemplar **currency**,
`.Codex/references/frontend-uplift-source-registry.md` §0 owns the freshness protocol and the
...
direction under `<repo>/.Codex/notes/frontend-designs/<slug>/plates/`, real labels and data
...
chosen thesis back into the repo overlay under `.Codex/references/frontend-uplift/`). **Persistence
...
Read **`.Codex/references/frontend-uplift-source-registry.md` §0** and apply its triggers **T1–T4**
...
```bash
SC_DIR="<repo-root>/.Codex/notes/frontend-designs/<slug>"   # run-scoped, local-only (gitignore .Codex/notes/ — never committed)
mkdir -p "$SC_DIR"
python3 .Codex/scripts/frontend-design-check.py template --slug <slug> > "$SC_DIR/scorecard.md"
```
...
```bash
python3 .Codex/scripts/frontend-design-check.py check "$SC_DIR/scorecard.md" --repo-root <repo-root>
...
# deliberate literal.
python3 .Codex/scripts/frontend-design-check.py diff-scan --repo-root <repo-root>
```
