...
## 2026-05-20T12:00:00Z · milestone:frontend-uplift-2026q2-m9 · status:complete (explore researcher)
- **Bottleneck observed:** Brief cited `.btn-primary` class for the CTA button — this class does NOT exist in the codebase; only `.modal-btn-primary` (Modal.css, modal-scoped) and `.btn-danger` (index.css, global) exist. Also: the brief implied one Gantt empty-state site, but there are two (`Gantt.tsx:94` for no-charts state and `ChartView.tsx:489` for no-tasks-in-chart state). Neither the Gantt add-task input nor the TodoList main input has an `id` or `ref` — both need one added for CTA focus.
- **What worked:** Grepping for `btn-primary` across all CSS files immediately confirmed absence; grepping `section-empty` across the entire src tree surfaced all 8+ usages and the two Gantt sites unambiguously.
- **What didn't:** n/a — codebase was clean once the grep enumerated the real sites.
- **Reusable lesson:** For empty-state illustration briefs, always grep `section-empty` across the full src tree (not just the expected file) — the same section often has multiple empty-state sites with different trigger conditions. Also grep the cited CSS class names before assuming they exist; proclivity uses `.modal-btn-primary` (Modal.css) and `.btn-danger` (index.css) but has no `.btn-primary` — implementer must add it.
