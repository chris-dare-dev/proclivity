# Critique — gemini-m1 — DEDUPED MERGE

**Sources:** adversary
**Counts:** C=0 H=1 M=2 L=0
...
- [HIGH] SECURITY.md not updated despite its own §2 update policy
- [MEDIUM] gemini-setup.md §2.3 skips scope registration on OAuth consent screen
- [MEDIUM] Key-generation command in setup.md §1 uses PKCS1 format, diverging from research doc §1.3
...
## What was done well
  ...
  - The manifest delta is minimal and correct: only the four fields required by the ACs were added (`key`, `identity` permission, `oauth2` block, `host_permissions`), with no speculative extras. The `"identity"` permission was correctly added (not `"identity.email"` or a broader variant).  _(adversary)_
  - The extension ID was independently verified by re-running the openssl pipeline against the committed `key` string; the result `cpflcminmnekdfjpmdhgblbolmclcdkk` matches the commit message's claim exactly. This is the hardest correctness property in the milestone to verify and it passes.  _(adversary)_
  - The `OAUTH_CLIENT_ID_PLACEHOLDER` constant pattern (`manifest.config.ts` line 20) is the right approach for a single-developer project: the build passes with the placeholder, the comment explains the lifecycle, and a future `gemini-m2` Settings UI can fail fast and clearly when the placeholder is still present.  _(adversary)_
  - The `*.pem` gitignore rule is broad and correct: it prevents both the primary key (`.proclivity-key.pem`) and any accidentally generated variants from entering git history, without being so specific that a renamed file slips through.  _(adversary)_
  - The commit is GPG-signed (`G` status), landed on `main` per project policy, has the correct conventional-commit prefix (`feat(build)`), a 43-char subject (within the 50-char limit), and the required co-author trailer.  _(adversary)_
  - No new npm dependencies were introduced. The bundle delta is purely manifest JSON (+0.69 kB), leaving all JS chunk sizes untouched. `npm run build` is reported clean with zero TypeScript errors.  _(adversary)_
  - `plans/gemini-setup.md` §1 correctly instructs the developer to confirm the PEM file is gitignored before proceeding — a simple but effective guard against accidental key leakage.  _(adversary)_
  - The "Open question — scope coverage" section at the end of `gemini-setup.md` is transparent about the `generative-language.retriever` vs `cloud-platform` ambiguity and gives a clear failure-mode (`403 Forbidden`) and fix (`add cloud-platform`). This matches the research synthesis's open question #1 guidance exactly.  _(adversary)_
  - The rotation/regeneration section in `gemini-setup.md` is clear that rotating the key changes the extension ID and requires re-registering the OAuth Client ID — a critical fact that is easy to forget and was explicitly surfaced.  _(adversary)_
  - External writes are correctly deferred: `state.json` lists them as `external_writes_required` but not `authorized` or `completed`, and the commit message explicitly acknowledges this boundary.  _(adversary)_
