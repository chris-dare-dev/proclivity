# Critique — frontend-uplift-2026q2-m4 — adversary

**Critic:** adversary
**Commit range:** d22031e..HEAD (64fb75b)
**Generated:** 2026-05-20T22:30:00Z
**Diff stats:** 4 files, +168/-7 LOC (3 production: App.tsx, App.css, react-augment.d.ts; 1 lessons.md)

## Verdict

SHIP-WITH-FIXES. The cross-dissolve implementation is correct in its core mechanics — the state machine mirrors the proven m5-s9 pattern, `inert` is wired as the load-bearing a11y guard, `position: relative` on `.content` is added, `animation-fill-mode: both` defends the incoming flash-of-content, and the build passes cleanly at 235.47 kB. However, one CSS specificity collision against `.section-empty` will visibly mis-animate the empty-state fallback (HIGH), the commit subject is 16 characters over the CLAUDE.md ≤50-char hard rule (HIGH), and the `motion` scope is not in the active CLAUDE.md scope list (MEDIUM, recurring drift from m5-M1). No CRITICAL findings; no test deltas (m1-L5 carry-over). Once the rectifier narrows the selector to `[role="tabpanel"]` and shortens the commit subject, this is ready.

## Executive summary

- [HIGH] `.content > div:not([hidden]):not([data-leaving]):not([data-staggered])` matches `<div class="section-empty">` when `visibleTabs.length === 0`, so the empty-state will run the 220 ms fade-in every time `rs.sectionVisibility` recomputes. Narrow the selector.
- [HIGH] Commit subject `feat(motion): section-fade cross-dissolve on tab switches (m4-s11)` is 52 chars after the `feat(motion): ` prefix and 66 chars total — CLAUDE.md hard rule is ≤50 chars after the prefix.
- [MEDIUM] Scope `motion` is not present in CLAUDE.md's active-scopes list (gantt, sprint, reminders, mesh, storage, build, a11y, skill, roadmap, docs, tune, style, perf, refactor, fix, feat). Same drift as m5-M1 (deferred); recurring.
- [MEDIUM] No test deltas for production-code change (m1-L5 carry-over). The cross-dissolve state machine has at least four edge cases worth a Vitest assertion (initial mount, rapid switching, timeout-cancellation, reduced-motion path).
- [LOW] The `transition: none` per-site reduced-motion guard is shadowed by `theme.css`'s `transition-duration: 0.01ms !important`. Both paths collapse correctly; the duplication exists for "audit clarity" only and is convention-consistent.
- [LOW] The `inert?: boolean | "" | undefined` union in `react-augment.d.ts` is wider than React's own React-19 declaration (`inert?: boolean | undefined`) — the empty-string member is unused and slightly invites the `inert=""` antipattern in callers.
- [LOW] The two `useLayoutEffect([tab])` blocks declared sequentially (stagger first, leavingTab second) work correctly but a single combined effect would halve the per-switch hook overhead and remove the implicit declaration-order dependency.
...
- The `prevTabRef` + `useLayoutEffect` Option A pattern is implemented exactly per synthesis §3.1, including the functional-updater `(current) => (current === prev ? null : current)` that prevents racing a more-recent `leavingTab` set from a subsequent tab change.
- `useLayoutEffect` (not `useEffect`) is correctly used to commit `data-leaving` + `inert` before paint, eliminating the same flash-of-attribute class that m5 rect M6 caught for the stagger.
- `position: relative` was added to `.content` (App.css:137) — the spike's #1 hard prerequisite per brief-1 §G3 and the m4 researcher's first lesson entry. Without this, every `[data-leaving]` panel would have escaped to `.app`.
- `animation-fill-mode: both` is correctly applied to the incoming-panel keyframe (App.css:174) — the LOAD-BEARING flash-of-content defense per synthesis §3.4. This is the single highest-leverage CSS detail in the diff.
- `inert` is wired as a JSX prop on all 7 tabpanels (App.tsx:495, 508, 521, 533, 545, 557, 589) with the `true | undefined` pattern, NOT `true | false` — avoiding the `inert="false"` truthy-string serialization trap. The implementer's synthesis §2.5 documents understanding of this nuance.
- The `react-augment.d.ts` module augmentation cleanly addresses the React 18.3 types gap with a delete-on-upgrade comment, preserving forward-compat with React 19 without polluting the runtime bundle (TypeScript-only file).
- The `:not([data-staggered])` carve-out on the incoming-panel selector correctly prevents the parent-fade × child-stagger compound-opacity competition for Today/Sprint/LongTerm — synthesis §3.5 prescription honored.
- Dual-guard reduced-motion blocks are present in both `[data-reduced-motion="true"]` form AND `@media (prefers-reduced-motion: reduce)` form, matching the sections.css convention even though the global theme.css reset would catch them. Convention discipline.
- Build verification matches the implementer's claim exactly: 235.47 kB initial chunk, +0.93 kB from m5 baseline, well under the 400 kB soft warn / 500 kB hard ceiling. Independent `npm run build` confirms.
- Zero CRITICAL findings, zero CSS keyframe collisions, zero new permissions, zero new chrome.storage writes, zero new external dependencies, zero MV3 service-worker touches, zero Node-only imports. The change is tightly scoped exactly as the synthesis predicted (~100 LOC, 2 production files + 1 type shim).

## Recommended rectification order

H1, H2, M1, M2, L2, L1, L3

(H1 is the only finding with user-visible behavioral consequence and should be first. H2 is a hard-rule violation but the rectifier must check push state before deciding amend-vs-tally. M1 and M2 are both recurring drifts worth opening tickets for. L2 is a one-character fix. L1 and L3 are flag-only; defer by default.)
