# Critique — gemini-nano-m1 — adversary

**Critic:** adversary
**Commit range:** 3597a50..b849eb0
**Generated:** 2026-05-12T03:00:00Z
**Diff stats:** 6 files changed, +487 / -5

## Verdict

SHIP-WITH-FIXES

The implementation is structurally sound: tsc exits 0 under all strict flags, no outbound network calls were added, no runtime dependencies were introduced, and six of seven formal ACs are cleanly satisfied. One HIGH bug corrupts the download-progress display (bytes used as if they were a 0–1 fraction), and one MEDIUM race condition produces a confusing TypeError on modal-close-during-prompt rather than a silent abort. Both are cheap two-line fixes that do not require rearchitecting the module.

## Executive summary

- [HIGH] `ProgressEvent.loaded` is bytes, not a 0–1 fraction; `Math.round(e.loaded * 100)` will display astronomically wrong percentages during the 4 GB model download. Fix: `e.loaded / e.total`.
- [MEDIUM] Race between modal-close abort and `session.prompt` call: `abortRef.current` is set to `null` by the close `useEffect` between `createSession` resolving and `session.prompt` reading `.signal`, producing a `TypeError` instead of a silent `AbortError`.
- [MEDIUM] `CreateSessionOpts.initialPrompts` is narrowed to `LanguageModelMessage[]`, silently dropping the `[LanguageModelSystemMessage, ...LanguageModelMessage[]]` overload the SDK supports. m2/m3 system-prompt seeding will require a revisit.
- [MEDIUM] `SettingsModal.tsx` grew to 292 lines; the Nano block (NanoState, NanoStatusBlock, NanoBadge, ~150 lines) is a clear extraction candidate into `src/components/NanoSettings.tsx`.
- [LOW] Commit subject is 64 chars after the `feat(llm):` prefix; CLAUDE.md mandates ≤ 50.
- [LOW] `params()` is referenced in `CreateSessionOpts.temperature` JSDoc but not exported; the @deprecated annotation in `@types/dom-chromium-ai@0.0.16` justifies the omission but the JSDoc reference is misleading.
- [LOW] `SECURITY.md` §3 says "Nothing is processed, transmitted, or replicated" — post-m1 user text IS processed (on-device by Nano). The "no transmission" claim remains true; the "not processed" wording is now imprecise.
- All 7 formal milestone ACs are satisfied (AC5 on progress display is partially satisfied — progress is shown but the value is wrong per the HIGH finding above).
...
- The `exactOptionalPropertyTypes`-safe pattern (conditional assignment of each optional field into `args`) is applied consistently and correctly throughout `createSession`; `tsc --noEmit` exits 0 with all strict flags active.
- `@types/dom-chromium-ai` is correctly placed in `devDependencies` — it emits zero runtime bytes and the build output confirms the 3.93 kB delta stays under the 5 kB AC cap.
- The abort/cleanup design correctly uses a cancellable `cancelled` flag in the availability `useEffect` and an `AbortController` ref for in-flight prompts; the two mechanisms are correctly independent.
- `NanoUnavailableError` and `NanoParseError` are typed error classes with distinct `.name` values, enabling precise `instanceof` checks in both the wrapper and callers.
- The `promptStructured<T>` helper is exported and compiles cleanly without exercising unverified schema features in m1's UI — the JSDoc explicitly calls out which keywords are unconfirmed, deferring empirical validation to m3.
- The unavailable-state hint block (SettingsModal.tsx:225–239) satisfies AC4 precisely: it explains the situation, links to `chrome://flags/#prompt-api-for-gemini-nano`, and includes an external docs link with `rel="noreferrer"`.
- The session lifecycle is disciplined: `session.destroy()` is in a `finally` block in both the one-shot `prompt()` helper and `promptStructured<T>()`, and in `SettingsModal.tsx`'s `runTestPrompt` function.
- No outbound `fetch` or `XMLHttpRequest` calls were introduced; `grep -rn "fetch\|XMLHttpRequest" src/` returns zero hits — the on-device constraint from SECURITY.md is preserved.
- The commit is GPG-signed (`G` status), carries the required `Co-Authored-By` trailer, and uses a valid conventional-commit type+scope prefix.
- The build chunk delta (+3.93 kB) is accurately reported in the commit message and verified by re-running `npm run build` — the claim is not aspirational.
