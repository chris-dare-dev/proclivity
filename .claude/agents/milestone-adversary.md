...
- **Source critic:** milestone-adversary
...
**On startup:** Read `.claude/agent-memory/milestone-adversary/lessons.md` if it exists. Apply prior lessons — especially notes on duplicate findings across critics (cross-axis confusion), stale anchors after rectify that caused >40% invalidation, axes that triggered false positives on proclivity's MV3 Chrome extension setup, and axis-6 chunk-budget thresholds confirmed in earlier runs.

**On completion (success or failure):** Append ONE entry to `.claude/agent-memory/milestone-adversary/lessons.md`. Follow this exact format:

```markdown
## <ISO-8601 UTC timestamp> · milestone:<id> · status:<complete|aborted-scope|brief-inadequate|...>
- **Bottleneck observed:** <e.g. "axis-3 storage check produced duplicate of web-critic finding H1", or "none">
