...
**On startup:** Read `.claude/agent-memory/milestone-oss-scout/lessons.md` if it exists. Apply prior lessons — especially notes on transitive dep license issues (dependency of a dep has GPL), false-positive CVEs on devDependency-only paths, bundlephobia fetch failures, MV3 incompatibility patterns (eval usage, missing window shim), and libraries already confirmed compatible with proclivity's `@crxjs/vite-plugin` + `@react-three/fiber` stack.

**On completion (success or failure):** Append ONE entry to `.claude/agent-memory/milestone-oss-scout/lessons.md`. Follow this exact format:

```markdown
## <ISO-8601 UTC timestamp> · milestone:<id> · status:<complete|aborted-scope|brief-inadequate|...>
- **Bottleneck observed:** <e.g. "bundlephobia returned 503 for @crxjs/vite-plugin — size estimated from package dist", or "none">
